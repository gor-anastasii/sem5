...
class Server {
    ...
    handleHeaders = (req, res) => {
        ...
        res.setHeader('X-UserHeader', 'TestValue');
    ...
    handleClose = (req, res) => {
        ...
        setTimeout(() => {
            server.close(() => {
                console.log('Server closed', process.uptime());
            });
    ...
    //http://localhost:5000/resp-status?code=278&mess=usp
    handleRespStatus = (req, res) => {
        ...
        if(Number(code)){
            if(+code >= 100 && code < 600) {
                ...
                res.end(`<p>Valid StatusCode is 100-599.</p>`)
            }
        }
        else{
            res.writeHead(407, 'Incorrect StatusCode', {'Content-Type': 'text/html; charset=utf-8'});
            res.end('<p>Correct StatusCode is 100-599.</p>')
        }
    }
    ...
    handleFiles = (req, res) => {
        ...
        if(file === undefined){
            fs.readdir('./static', (err, files) => {
                ...
                let counts = files.length;
                res.setHeader('X-static-files-count', `${counts}`);
                res.writeHead(200,  {'Content-Type': 'text/html; charset=utf-8'});
...
let serverHandle = ((req, res) => {
    ...
    if(req.method === 'GET'){
        switch(url.parse(req.url).pathname.split('/')[1]){
            case '':
                res.writeHead(200, {'Content-Type': 'text/html; chrset=utf-8'});
                res.end('<p>Try another pathame</p>');
                break;
            case 'connection': 
...
//     socket.on('close', () => {
//         console.log('Socket has beed closed');
//     })
})
